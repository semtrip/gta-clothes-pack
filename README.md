...
- Слот для имени `epic_cloth__…_<слот>_…`: префиксы компонента GTA (`jbib`, `uppr`, `lowr`, `p_head`, …) в строках drawable / имени файла; при отсутствии совпадения в имени используется `fallback_slot_slug` (по умолчанию `misc`), в отчёт всё равно попадает `unknown_slot`.
